...
| Growth/inbound | Warmbly intel + PNCP | scoreboard 404 = gap | NO_DATA if empty | separate | commercial.operations.growth | `/v1/domains/commercial?scope=commercial` + pncp | Crescimento (nine hops; missing = UNKNOWN/BLOCKED) | no | GSC/GA4 still BLOCKED without ingest | EXTERNAL_SERVICE_GATE |
| Clients | derived from Warmbly ops | projector | inherits commercial | inherits | clients v1 (`client_360=partial_warmbly_only`) | `/v1/domains/clients` | Clientes `#/clientes/<slug>` with Warmbly/Asaas/Governance cards | no | missing Asaas shown as UNKNOWN; not a full 360 | CREDENTIAL_GATE |
